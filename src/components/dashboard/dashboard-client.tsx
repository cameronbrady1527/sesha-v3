...
export function DashboardClient({ orgName, orgId }: DashboardClientProps) {
  ...
  // -------------------------- Filter Handlers ----------------------------
  function handleApplyFilters(appliedFilters: { timeRange?: RunFilters["timeRange"]; userId?: string; length?: string; sourceType?: "single" | "multi" }) {
    const newFilters = {
      ...appliedFilters,
  ...
  return (
    <div className="space-y-4">
      {/* Filters Section --- */}
      <div className="mt-6">
        <DashboardFilters filters={pendingFilters} users={users} onChange={handleApplyFilters} />
      </div>
      {/* Organization Name --- */}
      {orgName && <div className="text-sm font-medium text-muted-foreground mb-2">Organization: {orgName}</div>}
      {/* Data Table Section --- */}
      <DashboardDataTable data={dashboardRows} users={userOptions} />
    </div>
  );
}
